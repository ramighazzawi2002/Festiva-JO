    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
      {festivalData.festivals.map((festival) => (
        ...
        >
          {/* Image Section */}
          <img
            src={festival.image} // Assuming festival.image contains the image URL
            alt={festival.name}
            className="w-full h-48 object-cover"
          />
...
{/* <section ref={sectionRef} className="py-12 bg-page mx-4 md:mx-8 lg:mx-20">
  <div className="container mx-auto px-4 md:px-8 lg:px-12 flex flex-col items-center">
    <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-red1 mb-6 md:mb-8 text-center">
      Testimonials
    </h2>
    <p className="text-center mb-6 md:mb-8 max-w-2xl md:max-w-3xl text-gray-700">
      Explore the experiences of those who attended our cultural festivals. Hear from attendees about how these events have enriched their understanding of diverse cultures and provided unforgettable experiences.
    </p>
    <div className="relative w-full overflow-hidden">
      <div className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
        {testimonials.map((testimonial) => (
          <div
            key={testimonial.id}
            className="flex-shrink-0 w-full md:w-1/2 lg:w-1/3 p-4 md:p-6 text-center bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition duration-300"
          >
            <img
              src={testimonial.image}
              alt={testimonial.name}
              className="w-20 h-20 md:w-24 md:h-24 rounded-full mx-auto mb-4 border-4 border-red1"
            />
            <h3 className="text-lg md:text-xl font-semibold mb-2 text-red1">
              {testimonial.name}
            </h3>
            <p className="text-sm text-gray-500 mb-2 italic">
              {testimonial.role}
      ...
      <button
        className="absolute top-1/2 left-2 md:left-4 transform -translate-y-1/2 bg-red1 text-white p-2 md:p-3 rounded-full shadow-lg hover:bg-red2 focus:outline-none focus:ring-2 focus:ring-red-600"
        onClick={handlePrevClick}
      >
        &lt;
      </button>
      <button
        className="absolute top-1/2 right-2 md:right-4 transform -translate-y-1/2 bg-red1 text-white p-2 md:p-3 rounded-full shadow-lg hover:bg-red2 focus:outline-none focus:ring-2 focus:ring-red-600"
        onClick={handleNextClick}
      >
        &gt;
      </button>
